...
<div class="flex items-center gap-0.5" aria-label="${rating} of 5">
  <c:forEach var="starIndex" begin="1" end="5">
    <c:choose>
      <c:when test="${starIndex <= fullStars}">
        <span class="material-symbols-outlined ${resolvedSizeClass} leading-none icon-star-filled">star</span>
      </c:when>
      <c:otherwise>
        <span class="material-symbols-outlined ${resolvedSizeClass} leading-none icon-star-outline">star</span>
      </c:otherwise>
    </c:choose>
  </c:forEach>
</div>
