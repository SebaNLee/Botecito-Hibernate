...
<c:url var="outgoingUrl" value="/requests/outgoing" />
...
<c:url var="clearOutgoingFiltersUrl" value="/requests/outgoing">
...
<c:if test="${bookingPage.totalPages > 1}">
  <c:url var="previousPageUrl" value="/requests/outgoing">
  ...
  <c:url var="nextPageUrl" value="/requests/outgoing">
...
<paw:layout title="Botecito" mainClass="pt-24 pb-14 w-full max-w-7xl mx-auto px-6">
  <paw:toastNotifier />
  <form
      id="requests-outgoing-filters-form"
      action="${outgoingUrl}"
      method="get"
    ...
    <div class="relative z-40 w-full min-w-0 space-y-6">
      <paw:requestsSidebar active="outgoing" />
      <input type="hidden" name="page" value="1" id="outgoing-page-hidden" />
        ...
        <jsp:body>
          <div class="space-y-6">
            ...
            <div class="flex flex-col gap-3">
              ...
              <a href="${clearOutgoingFiltersUrl}" class="btn btn-outline btn-block no-underline">
    ...
    <section class="relative z-0 min-w-0 space-y-6">
      <div id="sent-booking-requests" class="scroll-mt-24 space-y-8">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:gap-4">
          ...
          <div class="flex shrink-0 flex-nowrap items-center gap-2 text-sm font-medium text-on-surface-variant lg:justify-end">
            <label for="requests-outgoing-sort" class="shrink-0 whitespace-nowrap"><c:out value="${sortLabel}" /></label>
            <select
                id="requests-outgoing-sort"
                name="sortBy"
                class="select select-sm w-32 max-w-[40vw] shrink-0 font-bold text-primary sm:max-w-none sm:w-36"
                onchange="document.getElementById('outgoing-page-hidden').value='1'; this.form.requestSubmit();">
            ...
            <label for="requests-outgoing-page-size" class="shrink-0 ml-2"><c:out value="${pageSizeFieldLabel}" /></label>
            <select
                id="requests-outgoing-page-size"
                name="pageSize"
        ...
        <div class="min-w-0">
          ...
          <h2 class="text-2xl font-extrabold tracking-tight text-on-background m-0 mt-6 break-words md:text-4xl">
            <spring:message code="profile.sentBookings.title" />
          </h2>
          ...
          <c:when test="${not empty bookings}">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4">
              <c:forEach var="b" items="${bookings}">
                ...
                <c:set var="detailModalId" value="nuevo-booking-detail-${b.id}" />
                  ...
                  <div class="flex min-w-0 flex-1 flex-col gap-1">
                    <div class="flex min-w-0 items-start gap-1.5">
                      <p class="m-0 min-w-0 flex-1 break-words text-xs font-extrabold text-on-surface line-clamp-2 sm:text-sm">
                        <spring:message code="requests.outgoing.card.listingVersion" arguments="${b.versionId}" />
                      </p>
          ...
          <c:otherwise>
            <div class="card bg-base-100 shadow-sm">
              <div class="card-body items-center gap-4 p-10 text-center">
                ...
                <p class="m-0 max-w-lg text-on-surface-variant"><spring:message code="profile.sentBookings.empty" /></p>
                <a href="${clearOutgoingFiltersUrl}" class="btn btn-primary btn-sm no-underline">
