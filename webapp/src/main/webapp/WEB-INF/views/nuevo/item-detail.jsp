...
<c:url var="currentVersionUrl" value="/nuevo/item/${item.id}">
  <c:if test="${not empty listingReturnTo}">
    <c:param name="returnTo" value="${listingReturnTo}" />
  </c:if>
</c:url>
    ...
    <aside class="order-1 lg:order-2 w-full min-w-0 lg:sticky lg:top-24 space-y-6">
      <div class="card bg-base-100 shadow-sm">
        <div class="card-body p-8 gap-4">
          ...
          <c:set var="pricePerHourInt" value="${item.price == null ? 0 : item.price.intValue()}" />
          <div
            class="hidden rounded-2xl bg-base-200 px-4 py-4"
            data-reservation-price-summary
            data-price-per-hour="${pricePerHourInt}"
          ...
          <c:choose>
            ...
            <c:when test="${isOwner}">
              ...
              <c:url var="ownerManageAvailabilityUrl" value="/profile/item/${item.id}/availability">
                <c:param name="return" value="/nuevo/item/${item.id}" />
              </c:url>
            ...
            <c:otherwise>
              ...
              >
                ...
                <c:if test="${not empty mailSuccessCode}">
                  <spring:message code="${mailSuccessCode}" arguments="${mailSuccessHostName}" var="mailSuccessMessage" />
                ...
                <button type="submit" class="btn btn-primary btn-block btn-lg relative mt-2" data-submit-loading-button>
