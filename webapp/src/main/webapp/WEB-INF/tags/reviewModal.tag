...
<c:choose>
  <c:when test="${existingReview != null}">
    <paw:detailsModal id="${modalId}" title="${reviewViewLabel}">
      <jsp:body>
        <div class="space-y-4">
          <paw:ratingStarsDisplay rating="${existingReview.rating}" />
      ...
      <jsp:body>
        ...
        <form action="<c:out value='${reviewPostUrl}' />" method="post" class="space-y-4">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-[8rem_minmax(0,1fr)] gap-3">
            ...
            <textarea id="review-comment-${bookingId}" name="comment" rows="3" maxlength="255" class="textarea textarea-bordered w-full"></textarea>
          </div>
