...
tagdir="/WEB-INF/tags" %> <%@ page contentType="text/html; charset=UTF-8"
pageEncoding="UTF-8" %>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-8 items-start">
      ...
      >
        <jsp:attribute name="title"
          ><spring:message code="publish.step3.summary.title"
        /></jsp:attribute>
        <jsp:body>
          <div
          ...
          >
            <c:choose>
              <c:when test="${not empty uploadedImagePreviewUrls}">
                <img
                  src="${uploadedImagePreviewUrls[0]}"
          ...
          <div class="grid grid-cols-2 gap-3">
            <div class="rounded-xl bg-base-200 p-3">
              ...
              <p class="text-sm font-bold text-on-surface mt-1 mb-0">
                <c:choose>
                  <c:when test="${publishForm.itemTypeId == '1'}"
                    ><spring:message code="publish.type.otros"
                  /></c:when>
                  <c:when test="${publishForm.itemTypeId == '2'}"
                    ><spring:message code="publish.type.kayak"
                  /></c:when>
                  <c:when test="${publishForm.itemTypeId == '3'}"
                    ><spring:message code="publish.type.paddle"
                  /></c:when>
                  <c:when test="${publishForm.itemTypeId == '4'}"
                    ><spring:message code="publish.type.canoa"
                  /></c:when>
                  <c:when test="${publishForm.itemTypeId == '5'}"
                    ><spring:message code="publish.type.windsurf"
                  /></c:when>
                  <c:when test="${publishForm.itemTypeId == '6'}"
                    ><spring:message code="publish.type.efoil"
                  /></c:when>
                  <c:when test="${publishForm.itemTypeId == '7'}"
            ...
            <div class="rounded-xl bg-base-200 p-3">
              ...
              <p class="text-sm font-bold text-on-surface mt-1 mb-0">
                <spring:message
                  ...
                  arguments="${publishForm.capacity}"
                />
              </p>
            </div>
            <div class="rounded-xl bg-base-200 p-3">
              ...
              <p class="text-sm font-bold text-on-surface mt-1 mb-0">
                $
                <fmt:formatNumber
                  value="${publishForm.pricePerHour}"
                  type="number"
                  groupingUsed="true"
                  maxFractionDigits="0"
                />
            ...
            <div class="rounded-xl bg-base-200 p-3">
              ...
              <p class="text-sm font-bold text-on-surface mt-1 mb-0">
                <c:out value="${selectedLocationName}" />
              </p>
            </div>
            <div class="rounded-xl bg-base-200 p-3 col-span-2">
              ...
              <p class="text-sm font-bold text-on-surface mt-1 mb-0">
                <c:choose>
                  <c:when test="${not empty publishForm.weight}"
                    ><fmt:formatNumber
                  ...
                  >
                  <c:otherwise
                    ><spring:message code="publish.form.weight.unspecified"
                  /></c:otherwise>
                </c:choose>
              </p>
            </div>
          ...
          <div>
            ...
            <c:choose>
              ...
              <c:otherwise>
                <p class="text-sm text-error m-0">
                  <spring:message code="publish.step3.summary.noAvailability" />
                </p>
              </c:otherwise>
            </c:choose>
          </div>
      ...
      <paw:sectionCard cssClass="lg:col-span-3" icon="person" hostAccent="true">
        <jsp:attribute name="title"
          ><spring:message code="publish.step3.account.title"
        /></jsp:attribute>
        <jsp:body>
          <div
