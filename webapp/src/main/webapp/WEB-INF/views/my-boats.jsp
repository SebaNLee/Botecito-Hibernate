          ...
          <div id="received-booking-requests" class="scroll-mt-24 space-y-4">
            ...
            <form action="<c:url value='/my-boats' />" method="get" class="flex flex-col gap-3 rounded-xl bg-base-200 p-4 sm:flex-row sm:items-end">
              <label class="form-control w-full sm:max-w-xs">
                <span class="label-text text-xs font-bold uppercase tracking-wider text-outline"><spring:message code="dashboard.filters.boat" /></span>
                <select name="boatId" class="select select-bordered select-sm">
                  <option value=""><spring:message code="dashboard.filters.boat.all" /></option>
                  <c:forEach var="item" items="${ownedItems}">
                    <option value="${item.id}" ${boatFilter == item.id ? 'selected="selected"' : ''}><c:out value="${item.title}" /></option>
                  </c:forEach>
                </select>
              </label>
              <label class="form-control w-full sm:max-w-xs">
            ...
            <c:if test="${receivedBookingPage.totalPages > 1}">
              <c:url var="receivedPreviousPageUrl" value="/my-boats">
                ...
                <c:if test="${not empty boatFilter}"><c:param name="boatId" value="${boatFilter}" /></c:if>
              </c:url>
