(function () {
  ...
  function bindInlineDraftGallery(root) {
    ...
    const badgeClass =
      root.dataset.galleryBadgeClass ||
      "badge badge-primary badge-sm font-bold";
    ...
    function bindInlineDraftInteractions() {
      ...
      previewList
        .querySelectorAll("[data-gallery-local-remove]")
        .forEach(function (button) {
          button.addEventListener("click", function (event) {
            event.preventDefault();
            const entryId = parseInt(
              button.getAttribute("data-gallery-local-remove"),
              10,
            );
            removeEntry(entryId);
          });
          ...
          item.addEventListener("dragstart", function (event) {
            ...
            if (event.dataTransfer) {
              ...
              event.dataTransfer.setData(
                "text/plain",
                item.getAttribute("data-gallery-key") || "",
              );
            }
  ...
  function init() {
    document
      .querySelectorAll("[data-image-gallery]")
      .forEach(bindInlineDraftGallery);
  }
