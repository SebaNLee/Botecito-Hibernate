...
<spring:message code="publish.success.createAnother" var="publishCreateAnotherLabel" />
<spring:message code="publish.success.goMarketplace" var="publishGoMarketplaceLabel" />
...
<paw:layout
  ...
  <div class="text-center mb-10">
    <div class="inline-flex items-center justify-center w-20 h-20 rounded-full bg-success/15 text-success mb-6">
  ...
  <paw:sectionCard cssClass="mb-8" icon="inventory_2">
    <jsp:attribute name="title"><spring:message code="publish.success.summary.title" /></jsp:attribute>
    <jsp:body>
      <div class="rounded-xl overflow-hidden border border-outline-variant/30 bg-base-200">
        <c:choose>
          <c:when test="${not empty itemImageUrl}">
            <img src="${itemImageUrl}" alt="<spring:message code='publish.image.previewAlt' />" class="w-full aspect-[16/10] object-cover" />
          </c:when>
          <c:otherwise>
  ...
  <div class="flex flex-col sm:flex-row justify-center items-center gap-4">
    <paw:button href="${publishUrl}" color="primary" size="lg" icon="add" cssClass="w-full sm:w-auto" text="${publishCreateAnotherLabel}" />
    <paw:button href="${marketplaceUrl}" color="outline" size="lg" icon="storefront" cssClass="w-full sm:w-auto" text="${publishGoMarketplaceLabel}" />
  </div>
