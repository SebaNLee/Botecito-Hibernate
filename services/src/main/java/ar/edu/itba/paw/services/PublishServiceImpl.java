...
public class PublishServiceImpl implements PublishService {
    ...
    public void create(
        ...
        final Version version = itemService.create(
                ownerId,
                typeId,
                title,
                description,
    ...
    private static boolean sameAvailabilities(
        ...
        for (final AvailabilityWindow window : submitted) {
            if (window == null || window.getWeekday() == null) {
                continue;
            }
    ...
    private static boolean sameImages(final Version current, final List<ImageUpload> submitted) {
        if (submitted == null || submitted.stream().anyMatch(upload -> upload != null && !upload.isExisting())) {
            return false;
        }
        return currentImageIds(current)
                .equals(submitted.stream()
                        .filter(Objects::nonNull)
                        .map(ImageUpload::getExistingImageId)
                        .toList());
    }
