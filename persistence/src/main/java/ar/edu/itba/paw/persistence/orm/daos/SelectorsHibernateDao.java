...
public class SelectorsHibernateDao implements SelectorsDao {
    ...
    public List<LocationOrm> getLocationOptions() {
        final TypedQuery<LocationOrm> query = entityManager.createQuery(
                "SELECT l FROM LocationOrm l ORDER BY l.id", LocationOrm.class);
        return query.getResultList();
    }

    @Override
    public List<ItemTypeOrm> getItemTypeOptions() {
        final TypedQuery<ItemTypeOrm> query = entityManager.createQuery(
                "SELECT t FROM ItemTypeOrm t ORDER BY t.name", ItemTypeOrm.class);
        return query.getResultList();
    }
}
